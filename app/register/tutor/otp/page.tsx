...
export default function TutorOtpPage() {
  ...
  function handleResend() { setCountdown(60); setCanResend(false); setOtp(["", "", "", "", "", ""]); document.getElementById("otp-0")?.focus(); }

  function handleOtp(val: string, i: number) {
    if (!/^\d?$/.test(val)) return;
    const u = [...otp]; u[i] = val; setOtp(u);
    ...
    <div className="flex min-h-screen">
      ...
      <div className="w-1/2 bg-[#fafafa] flex items-center justify-center">
        <div className="w-full max-w-md px-10 flex flex-col gap-4">
          ...
          <div className="mb-2">
            <p className="text-xs font-medium tracking-[0.15em] uppercase mb-3" style={{ color: PRIMARY }}>Verification</p>
            <h1 className="text-[1.75rem] font-bold tracking-tight text-[#1a1c1e] leading-snug">Enter the code<br />we sent you</h1>
          ...
          <div className="flex items-center justify-between">
            {canResend
              ? <button onClick={handleResend} className="flex items-center gap-1.5 text-[12.5px] font-semibold" style={{ color: WA_GREEN }}><WhatsappLogo size={15} weight="fill" />Resend via WhatsApp</button>
              : <p className="text-[12px] text-[#9a9aa8]">Resend in <span className="font-semibold tabular-nums" style={{ color: PRIMARY }}>{countdown}s</span></p>
            }
            <p className="text-[12px] text-[#9a9aa8]">Didn&apos;t receive it?</p>
          </div>

          <button
            onClick={() => router.push("/register/tutor/password")}
            disabled={!filled}
