...
const LEFT_PANEL = {
  academic:      { line1: "Pick your", line2: "subjects.",  sub: "Choose everything you want to learn — no limits." },
  "non-academic":{ line1: "Pick your", line2: "passions.",  sub: "Choose everything you want to explore — no limits." },
};
...
function SubjectsContent() {
  ...
  function handleContinue() {
    ...
    router.push(`/onboard/level?category=${category}&subjects=${encodeURIComponent(list)}`);
  }
    ...
    <div className="flex min-h-screen">
      ...
      <div className="w-1/2 flex flex-col px-12 py-10" style={{ backgroundColor: PRIMARY_LIGHT }}>
        ...
        <div className="flex flex-col items-center justify-center flex-1 gap-8 text-center">
          ...
          {selected.size > 0 && (
            <div className="flex items-center gap-2 px-4 py-2 rounded-full" style={{ backgroundColor: `${PRIMARY}15` }}>
              <CheckCircle size={16} weight="fill" color={PRIMARY} />
              <span className="text-[13px] font-semibold" style={{ color: PRIMARY }}>{selected.size} selected</span>
            </div>
          )}
        </div>
      </div>
            ...
            {subjects.map(({ label, icon }) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-3 px-4 py-3.5 rounded-[16px] text-left transition-all duration-200"
                  style={{
                    border: `1.5px solid ${isOn ? PRIMARY : "#e1e2ec"}`,
                    backgroundColor: isOn ? PRIMARY_LIGHT : "#ffffff",
                  }}
                >
                  <span className="material-symbols-rounded text-[20px] shrink-0" style={{ color: isOn ? PRIMARY : "#8888aa" }}>
                    {icon}
                  </span>
                  <span className="text-[13px] font-medium leading-tight flex-1 transition-colors duration-200" style={{ color: isOn ? PRIMARY : "#1a1c1e" }}>
                    {label}
                  </span>
...
export default function SubjectsPage() {
  return <Suspense><SubjectsContent /></Suspense>;
}
